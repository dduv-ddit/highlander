...
public class CreateHighlightCriterion extends JDialog {
	
	private HighlightCriterion criterion = null;
	...
	private JRadioButton radioButton_rangeII;
	private JRadioButton radioButton_rangeIE;
	private JRadioButton radioButton_rangeEI;
	private JRadioButton radioButton_rangeEE;
	...
	private JCheckBox boxExpandRow;

	AutoCompleteSupport<Field> support;
	
	public CreateHighlightCriterion(Analysis analysis, HighlightingPanel highlightingPanel, HighlightCriterion highlighting) {
		this.analysis = analysis;
		this.highlighting = highlightingPanel;
		this.variantsTable = highlightingPanel.getVariantsTable();
	...
	public CreateHighlightCriterion(Analysis analysis, HighlightingPanel highlightingPanel) {
	...
	private void initUI(){
		...
		radioButton_rangeII = new JRadioButton(ComparisonOperator.RANGE_II.getUnicode());
		radioButton_rangeII.setToolTipText("Range [min;max] - just give min and max separated by ;");
		comparisonButtonGroup.add(radioButton_rangeII);
		panel_1.add(radioButton_rangeII);
		
		radioButton_rangeIE = new JRadioButton(ComparisonOperator.RANGE_IE.getUnicode());
		radioButton_rangeIE.setToolTipText("Range [min;max[ - just give min and max separated by ;");
		comparisonButtonGroup.add(radioButton_rangeIE);
		panel_1.add(radioButton_rangeIE);
		
		radioButton_rangeEI = new JRadioButton(ComparisonOperator.RANGE_EI.getUnicode());
		radioButton_rangeEI.setToolTipText("Range ]min;max] - just give min and max separated by ;");
		comparisonButtonGroup.add(radioButton_rangeEI);
		panel_1.add(radioButton_rangeEI);
		
		radioButton_rangeEE = new JRadioButton(ComparisonOperator.RANGE_EE.getUnicode());
		radioButton_rangeEE.setToolTipText("Range ]min;max[ - just give min and max separated by ;");
		comparisonButtonGroup.add(radioButton_rangeEE);
		panel_1.add(radioButton_rangeEE);
		...
		btnBackground = new JButton("Background",Resources.getColoredSquare(16, (backgroundColor == null ? new Color(0,0,0,0) : backgroundColor)));
		btnBackground.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				Color chosen = JColorChooser.showDialog(null, "Background color", backgroundColor);
				if (chosen != null){
					backgroundColor = chosen;
					btnBackground.setIcon(Resources.getColoredSquare(16, backgroundColor));
				}
			}
		});
		panel_3.add(btnBackground);		
		
		btnForeground = new JButton("Foreground",Resources.getColoredSquare(16, (foregroundColor == null ? Color.BLACK : foregroundColor)));
		btnForeground.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				Color chosen = JColorChooser.showDialog(null, "Foreground color", foregroundColor);
				if (chosen != null){
					foregroundColor = chosen;
					btnForeground.setIcon(Resources.getColoredSquare(16, foregroundColor));
				}
			}
		});
		panel_3.add(btnForeground);		
		
		boxBold = new JCheckBox("Bold");
		panel_3.add(boxBold);		
		
		boxItalic = new JCheckBox("Italic");
		panel_3.add(boxItalic);		
		
		boxExpandRow = new JCheckBox("Expand to row");
		panel_3.add(boxExpandRow);		
	...
	private void setAvailableComparisonOperators(){
		try{
			...
			if (comboBox_field.getSelectedItem() != null){
				...
				availableOps.add(ComparisonOperator.RANGE_EE);
				availableOps.add(ComparisonOperator.RANGE_EI);
				availableOps.add(ComparisonOperator.RANGE_IE);
				availableOps.add(ComparisonOperator.RANGE_II);
			}
			if(chkBox_null.isSelected()){
				...
				availableOps.remove(ComparisonOperator.RANGE_EE);
				availableOps.remove(ComparisonOperator.RANGE_EI);
				availableOps.remove(ComparisonOperator.RANGE_IE);
				availableOps.remove(ComparisonOperator.RANGE_II);
			}
			...
			radioButton_rangeEE.setEnabled((availableOps.contains(ComparisonOperator.RANGE_EE)));
			radioButton_rangeEI.setEnabled((availableOps.contains(ComparisonOperator.RANGE_EI)));
			radioButton_rangeIE.setEnabled((availableOps.contains(ComparisonOperator.RANGE_IE)));
			radioButton_rangeII.setEnabled((availableOps.contains(ComparisonOperator.RANGE_II)));
			if (!availableOps.contains(getSelectedComparisonOperator())){
				...
				case DOESNOTCONTAINS:
					...
					break;
				case RANGE_EE:
					radioButton_rangeEE.setSelected(true);
					break;
				case RANGE_EI:
					radioButton_rangeEI.setSelected(true);
					break;
				case RANGE_IE:
					radioButton_rangeIE.setSelected(true);
					break;
				case RANGE_II:
					radioButton_rangeII.setSelected(true);
					break;
				default:
		...
		if (criterion != null) {
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					...
					if (criterion.getBackground() != null){
						backgroundColor = criterion.getBackground();
						btnBackground.setIcon(Resources.getColoredSquare(16, backgroundColor));
					}
					if (criterion.getForeground() != null){
						foregroundColor = criterion.getForeground();
						btnForeground.setIcon(Resources.getColoredSquare(16, foregroundColor));
					}
					boxBold.setSelected(criterion.isBold());
					boxItalic.setSelected(criterion.isItalic());
					boxExpandRow.setSelected(criterion.expandRow());
				}
			});
		}
	}
	...
	private boolean hasSingleValue(){
		return !(radioButton_equal.isSelected() || radioButton_different.isSelected()) && !hasDualValues();
	}
	
	private boolean hasDualValues(){
		return (radioButton_rangeII.isSelected() || radioButton_rangeIE.isSelected() || radioButton_rangeEI.isSelected() || radioButton_rangeEE.isSelected());
	}
	...
	private ComparisonOperator getSelectedComparisonOperator(){
		...
		}else if (radioButton_rangeII.isSelected()){
			compop = ComparisonOperator.RANGE_II;				
		}else if (radioButton_rangeEI.isSelected()){
			compop = ComparisonOperator.RANGE_EI;				
		}else if (radioButton_rangeIE.isSelected()){
			compop = ComparisonOperator.RANGE_IE;				
		}else if (radioButton_rangeEE.isSelected()){
			compop = ComparisonOperator.RANGE_EE;				
		}
		return compop;
	}
	...
	private boolean generateCriterion(){		
		try {
			...
			boolean expandRow = boxExpandRow.isSelected();
			if (chkBox_null.isSelected()){
				criterion = new HighlightCriterion(highlighting, field, compop, true, new ArrayList<String>(), new ArrayList<String>(), backgroundColor, foregroundColor, bold, italic, expandRow);
			}else{
	...
	public HighlightCriterion getCriterion(){
		return criterion;
	}
